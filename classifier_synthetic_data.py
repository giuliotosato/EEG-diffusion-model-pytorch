...
for model_seed in os.listdir('classifier_model_saved_original_data'):
    model = torch.load(f'/home/u956278/openai/openai_happy/classifier_model_saved_original_data/{model_seed}',
                       map_location=torch.device('cpu'))

    results[model_seed] = {}
    results[model_seed]["epoch"] = []
    results[model_seed]["correctly_predicted_happy"] = []
    results[model_seed]["correctly_predicted_sad"] = []
    results[model_seed]["correctly_predicted_total"] = []
    results[model_seed]["mislabelled"] = []
    results[model_seed]["accuracy"] = []
    ...
    for i in range(0, 14):
        ...
        for dir in [dir1, dir2]:
            for file in os.listdir(dir):
                ...
                valX.append(np.array(Image.fromarray(image))[14:114, 33:95])
        valX = np.array(valX)
        ...
        for s, tensor in enumerate(y_pred):
            if tensor[0] < tensor[1] and s <= 256:
                dir1_count += 1
            elif tensor[0] > tensor[1] and s > 256:
                dir2_count += 1
            else:
                misslabeled += 1
        ...
        results[model_seed]["epoch"].append(i)
        results[model_seed]["correctly_predicted_happy"].append(dir1_count)
        results[model_seed]["correctly_predicted_sad"].append(dir2_count)
        results[model_seed]["correctly_predicted_total"].append(dir1_count + dir2_count)
        results[model_seed]["mislabelled"].append(misslabeled)
        results[model_seed]["accuracy"].append((dir2_count + dir1_count) / (256 * 2))

with open('epochs_accuracy_graphs_data.pickle', 'wb') as handle:
