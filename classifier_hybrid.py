...
file_path_3 = ('/home/u956278/openai/openai_happy/syntethic_images/sad/images_5')
file_path_4 = ('/home/u956278/openai/openai_happy/syntethic_images/happy/images_5')


# Load images as numpy arrays
def load_images(file_path_1):
    images = []
...
def load_labels_sad(X):
    labels_sad = [0 for i in range(X.shape[0])]
    return labels_sad


def load_labels_happy(X):
    labels_happy = [1 for i in range(X.shape[0])]
    return labels_happy
...
for randomState in list(np.random.randint(0, high=2 ** 32 - 1, size=20)):
    ...
    X = load_images(file_path_1)
    print('splitting images')
    X_train1, X_test1, y_train1, y_test1 = train_test_split(X, load_labels_sad(X), test_size=0.2,
                                                            random_state=randomState, shuffle=False)
    print('loading images')
    X = load_images(file_path_2)
    print('splitting images')
    X_train2, X_test2, y_train2, y_test2 = train_test_split(X, load_labels_happy(X), test_size=0.2,
                                                            random_state=randomState, shuffle=False)
    X_3 = load_images(file_path_3)
    print('splitting images')
    load_labels_sad(X_3)
    X_4 = load_images(file_path_4)
    print('splitting images')
    load_labels_happy(X_4)
    X_train = np.concatenate((X_train1, X_train2, X_3, X_4))
    y_train = np.concatenate((y_train1, y_train2, load_labels_sad(X_3), load_labels_happy(X_4)))
    ...
    print(y_train[0:100], y_train[-100:])
    print(y_test[0:100], y_test[-100:])
    train_dataset = ImageDataset(np.rollaxis(np.array(X_train), 3, 1), np.array(y_train))
    val_dataset = ImageDataset(np.rollaxis(np.array(X_test), 3, 1), np.array(y_test))
...
with open('hybrid_graphs_data_5.pickle', 'wb') as handle:
